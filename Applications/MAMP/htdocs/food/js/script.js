...
window.addEventListener('DOMContentLoaded', () => {
  ...
  function getTimeRemaining(endtime) {
    const t = Date.parse(endtime) - Date.parse(new Date()),
          ...
          //оператор % делит выражение на 24 и отдает остаток
    minutes = Math.floor(t / 1000 / 60 % 60),
          seconds = Math.floor(t / 1000 % 60);
    return {
      // чтобы вернуть переменные наружу используем оператор return, но при этом возвращаем объект
      'total': t,
      'days': days,
      'hours': hours,
      'minutes': minutes,
  ...
  function setClock(selector, endtime) {
    // функция которая установит часы на страницу
  ...
  setClock('.timer', deadline); // Modal____________________________________________________________________________________

  const modalBtn = document.querySelectorAll('[data-modal]'),
        // data-modal кастомный атрибут
  ...
  modalBtn.forEach(btn => {
    // перебираем кнопки в псевдомассиве
  ...
  modalWindow.addEventListener('click', e => {
    // закрытие модального окна кликом на подложку, любое пространство вне модального окна
    if (e.target === modalWindow || e.target.getAttribute('data-close') == '') {
      //если клик произведен на подложку или крестик
  ...
  document.addEventListener('keydown', e => {
    ...
    if (e.code === 'Escape' && modalWindow.classList.contains('show')) {
      //если нажата кнопка esc и проверка что у модального окна есть класс show, то есть оно открыто
  ...
  function showModalByScroll() {
    // выпрыгивающее мод окно если пользователь долистал до конца страницы
  ...
  function showThanksModal(message) {
    ...
    setTimeout(() => {
      // удаление блока thanksModalWrapper чтобы клиент снова мог отправить заявку
